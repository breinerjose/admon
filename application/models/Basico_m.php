...
class Basico_m extends CI_Model {	
	...
	function asignado($campos,$tabla,$condicion,$condicionb){//
		$this->db->select($campos);
		$this->db->from($tabla);
		$this->db->where($condicion);
		...
		$res = $this->db->get();
		//echo $this->db->last_query(); exit();
		if($res->num_rows()>0){return 1;}else{return '';}
	}
	...
	function Consecutivo_factura($coddoc){
		$this->db->select('max(nrocmp::numeric)+1 as nrocmp');
		$this->db->from('cntcmp');
		$this->db->where(array('coddoc'=>$coddoc));
		$res = $this->db->get();
		if($res->num_rows()>0){
		$r = $res->row_array();
		return $r['nrocmp'];
