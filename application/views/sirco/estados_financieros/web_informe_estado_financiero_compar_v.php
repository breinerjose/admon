...
if ( isset($pdf) ) { 
    $pdf->page_script('
            ...
            $size = 9;
            $pageText = "<?php echo encodeUtf8(encodeUtf8($footer['pie1']));?>";
            $y = $pdf->get_height() - 50;
            $x = ($pdf->get_width() /2) - ( Font_Metrics::get_text_width($pageText, $font, $size) / 2);
            $pdf->text($x, $y, $pageText, $font, $size);
			
		    $pageText = "<?php echo encodeUtf8(encodeUtf8($footer['pie2']));?>";
            $y = $pdf->get_height() - 40;
            $x = ($pdf->get_width() /2) - ( Font_Metrics::get_text_width($pageText, $font, $size) / 2);
            $pdf->text($x, $y, $pageText, $font, $size);
			...
			$pageText = "<?php echo encodeUtf8(encodeUtf8($footer['pie3']));?>";
            $y = $pdf->get_height() - 30;
            $x = ($pdf->get_width() /2) - ( Font_Metrics::get_text_width($pageText, $font, $size) / 2);
            $pdf->text($x, $y, $pageText, $font, $size);
			
			$pageText = "Reembolso De ";
            $y = 100;
			$size = 12;
            $x = ($pdf->get_width() /2) - ( Font_Metrics::get_text_width($pageText, $font, $size) / 2);
            $pdf->text($x, $y, $pageText, $font, $size);
			...
			$img_w = 100; 
			$img_h = 90; 
			$pdf->image("./res/icons/05/tecnar.png",  40 , $y - $img_h, $img_w, $img_h );
			
			$w = $pdf->get_width(); 
			$y = 120; 
			$img_w = 100; 
			$img_h = 90; 
			$pdf->image("./res/icons/05/acreditacion.png",  450 , $y - $img_h, $img_w, $img_h );
	   ');
