 <div class="container">
	...
	<div class="row">
            <div class="col-md-8">
                <div class="form-group">
                    <label>Sede:</label>
					<input type="hidden" id="coddoc" name="coddoc" value="02">
	   ...
	   //enter
		...
		if($('#doblectd').is(':checked')){ ctaknp2 = $('#ctaknp2').val(); vlrcta2=vlrcta;}
		 ...
		 if(codtrc!='' && ctaknp!='' && vlrcta!=''){
				$.ajax({
							  url:"/sirco/Egresos_c/agregarConceptos/",
							  data:{'codtrc':codtrc,'ctaknp':ctaknp,'vlrcta':vlrcta,'fecha':fecha},
							  type:"POST",
							  success: function(resp){
								 if(resp==1){
									...
									///////
									$.ajax({
									  url:"/sirco/Egresos_c/agregarConceptos2/",
									  data: {'codtrc':codtrc,'ctaknp2':ctaknp2,'vlrcta2':vlrcta2,'fecha':fecha},
									  type:"POST",
									  dataType:"json",
									  success: function(resp){
										 if(resp==1){
		...
		codsed = $('#codsed').val();
		 ...
		 if(codtrc!='' && ctaknp2!='' && vlrcta2!=''){
						$.ajax({
						  url:"/sirco/Egresos_c/agregarConceptos2/",
						  data: {'codtrc':codtrc,'ctaknp2':ctaknp2,'vlrcta2':vlrcta2,'fecha':fecha,'codsed':codsed},
						  type:"POST",
						  dataType:"json",
						  success: function(resp){
							 if(resp==1){
    ...
    $('#coddoc').append('<option value="' + v.coddoc + '">' + v.coddoc + ' - ' + v.nomdoc + '</option>');
		...
		function credito(){
		$.post('/sirco/Ingresos_c/debito/', function (resp) {
           $.each(resp, function (i, v) {
